...
Use this guide when creating, extracting, updating, or reviewing `SRS.md`. Also apply `reference/STYLE.md` for headers, IDs, versioning, traceability, and table conventions.

## Purpose and Audience

Turns PRD features into testable functional and non-functional requirements. Write for implementers, reviewers, and future agents who need clear scope, source evidence, and testable decisions.

## When to Include or Skip

Include `SRS.md` when the selected archetype or change request needs this artifact. Skip it when it would only repeat another doc without adding implementation or verification value. If skipped, record the reason in PRD §Doc Set.

## Inputs to Read First

- PRD features, architecture components, business rules, use cases, brownfield behavior/tests.
...
- Requirements must be testable, atomic, and implementation-aware without prescribing code internals.
- Split functional requirements (`FR-*`) from non-functional requirements (`NFR-*`).
- Each `FR-*` should cite the PRD feature and implementing `C-*`.
- Link requirements to related workflows (`UF-*`) or actor/system sequences when the behavior is easier to verify in context.
- Include measurable thresholds for performance, reliability, security, usability, observability, and compatibility.
...
A strong `SRS.md` normally contains:
...
Owns `FR-*` and `NFR-*`. User stories, use cases, API endpoints, database behavior, and tests trace back here.

## Good Example Snippet

See `examples/srs-example.md`. Requirements should be atomic and testable:

```markdown
### FR-TASK-001 Normalize task intake
The system shall require title, source, priority, and owner candidate before a task enters triage.
Implemented by: C-01.
Related workflow: UF-01.
```
