...
Use this guide when creating, extracting, updating, or reviewing `USECASES.md`. Also apply `reference/STYLE.md` for headers, IDs, versioning, traceability, and table conventions.

## Purpose and Audience

Documents actor-goal interactions, main success scenarios, extensions, and postconditions. Write for implementers, reviewers, and future agents who need clear scope, source evidence, and testable decisions.

## When to Include or Skip

Include `USECASES.md` when the selected archetype or change request needs this artifact. Skip it when it would only repeat another doc without adding implementation or verification value. If skipped, record the reason in PRD §Doc Set.

## Inputs to Read First

- Personas/actors, SRS requirements, business rules, UI/API entry points, support workflows.
...
- Prefer Cockburn-style actor-goal interactions: primary actor, trigger, preconditions, main success scenario, extensions, postconditions.
- Keep use cases at user-goal level; do not turn every button click into a use case.
- Use extensions for errors, authorization failures, and alternate paths.
- For important or timing-sensitive main scenarios, include an actor-system Mermaid `sequenceDiagram` after the written steps.
- If both use cases and user stories exist, make one behavior view primary and cross-reference the other tersely.
...
A strong `USECASES.md` normally contains:
...
Owns `UC-*`. User flows and tests cite use cases; SRS requirements should be covered by at least one use case when included.

## Good Example Snippet

See `examples/use-cases-example.md`. Good use cases emphasize actor intent and alternatives:

```markdown
UC-TASK-01 Triage new task
Actor: Support coordinator
Extension 3a: Priority missing — block save and name the missing field.
```
...
sequenceDiagram
  actor Coordinator
  participant System
  Coordinator->>System: Submit task metadata
