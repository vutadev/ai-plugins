...
Use this guide when creating, extracting, updating, or reviewing `TECHSTACK.md`. Also apply `reference/STYLE.md` for headers, IDs, versioning, traceability, and table conventions.

## Purpose and Audience

Records technology choices, pinned versions, alternatives, licenses, lock files, and supply-chain notes. Write for implementers, reviewers, and future agents who need clear scope, source evidence, and testable decisions.

## When to Include or Skip

Include `TECHSTACK.md` when the selected archetype or change request needs this artifact. Skip it when it would only repeat another doc without adding implementation or verification value. If skipped, record the reason in PRD §Doc Set.

## Inputs to Read First

- PRD constraints, repo manifests, lock files, Docker/IaC, user technology decisions.
...
- Record concrete choices with pinned versions, not generic categories. Include lock-file evidence when available.
- For each `TS-*`, include rationale, alternatives rejected, license, and source.
- Separate what the project uses from where docs live; when `EXTERNAL_DOCS.md` exists, cite `EXT-*` instead of raw vendor URLs.
- Do not infer future stack choices from aspirations; mark uncertain choices as Proposed or Open Question.
...
A strong `TECHSTACK.md` normally contains:
...
Owns `TS-*` IDs. Architecture components cite `TS-*`; SRS cites stack only when behavior/constraints are user-observable.

## Good Example Snippet

See `examples/techstack-example.md`. Prefer pinned, sourced choices:

```markdown
| TS-DATA-01 | SQLite | 3.x | Single-file local persistence | package-lock / user decision |
Rejected: Postgres deferred because hosted collaboration is out of scope.
```
