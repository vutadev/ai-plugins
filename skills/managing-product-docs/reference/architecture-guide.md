...
Use this guide when creating, extracting, updating, or reviewing `ARCHITECTURE.md`. Also apply `reference/STYLE.md` for headers, IDs, versioning, traceability, and table conventions.

## Purpose and Audience

Explains system shape: context, containers, components, data flow, deployment, integrations, ADR-lite decisions, and tradeoffs. Write for implementers, reviewers, and future agents who need clear scope, source evidence, and testable decisions.

## When to Include or Skip

Include `ARCHITECTURE.md` when the selected archetype or change request needs this artifact. Skip it when it would only repeat another doc without adding implementation or verification value. If skipped, record the reason in PRD §Doc Set.

## Inputs to Read First

- PRD, TECHSTACK, repo structure, deployment files, route handlers, workers, modules, diagrams inferred from code.
...
- Explain boundaries: process, module, data ownership, network/API, queue/job, and deployment boundaries.
- Use `C-*` for components and `AD-*` for decisions. Every ADR needs context and consequences.
- Use C4-lite context/container/component diagrams when ownership or deployment boundaries are the main question.
- Use `sequenceDiagram` for cross-component request, auth, async job, or error-recovery timing.
- Use `flowchart TD` for data flow, state transitions, dependency chains, or deployment paths.
- Do not restate every requirement; architecture explains where behavior lives and why tradeoffs were chosen.
...
A strong `ARCHITECTURE.md` normally contains:
...
Owns `C-*` and `AD-*`. SRS requirements cite implementing `C-*`; API and Database docs cite components that own contracts/data.

## Good Example Snippet

See `examples/architecture-example.md`. A useful component entry names ownership and boundaries:

```markdown
### C-01 Task Intake Service
Responsibility: validate incoming task metadata.
Owned data: db.tasks, db.task_events.
Related: FR-TASK-001, API-TASK-001.
```
...
sequenceDiagram
  ...
  API->>DB: Persist normalized task
  API-->>UI: 201 Created or validation error
```
