...
Use this guide when creating, extracting, updating, or reviewing `BUSINESS_RULES.md`. Also apply `reference/STYLE.md` for headers, IDs, versioning, traceability, and table conventions.

## Purpose and Audience

Captures durable policy decisions that should survive UI/API/code rewrites. Write for implementers, reviewers, and future agents who need clear scope, source evidence, and testable decisions.

## When to Include or Skip

Include `BUSINESS_RULES.md` when the selected archetype or change request needs this artifact. Skip it when it would only repeat another doc without adding implementation or verification value. If skipped, record the reason in PRD §Doc Set.

## Inputs to Read First

- PRD constraints, compliance notes, stakeholder policy statements, existing validation/rules code.
...
- Write rules as durable policy: SHALL, MUST, MUST NOT, MAY only when meaningful.
- Include enforcement point, violation behavior, rationale, and related `FR-*`/`TC-*`.
- Keep transient UI copy and implementation algorithms out unless they express policy.
- Add conflict-resolution order when rules can collide.
...
A strong `BUSINESS_RULES.md` normally contains:
...
Owns `BR-*`. SRS/USER_STORIES/TESTCASES cite rules that constrain behavior.

## Good Example Snippet

See `examples/business-rules-example.md`. Rules should include enforcement and violation behavior:

```markdown
### BR-TASK-01 Priority must be explicit before assignment
Enforcement: task intake validation.
Violation: reject assignment and name the missing priority field.
```
