...
Use this guide when creating, extracting, updating, or reviewing `EXTERNAL_DOCS.md`. Also apply `reference/STYLE.md` for headers, IDs, versioning, traceability, and table conventions.

## Purpose and Audience

Maintains a registry of external APIs, SDK docs, standards, service docs, compliance references, and link freshness. Write for implementers, reviewers, and future agents who need clear scope, source evidence, and testable decisions.

## When to Include or Skip

Include `EXTERNAL_DOCS.md` when the selected archetype or change request needs this artifact. Skip it when it would only repeat another doc without adding implementation or verification value. If skipped, record the reason in PRD §Doc Set.

## Inputs to Read First

- TECHSTACK dependencies, architecture integrations, code imports, README links, vendor docs, standards URLs.
...
- Store pointers and metadata, not copied vendor documentation.
- Each `EXT-*` needs provider, URL, version/revision, last verified date, type, why we depend, auth/access, and cross-refs.
- Flag entries older than six months for link freshness review.
- Use this doc to prevent stale scattered links in TECHSTACK/ARCHITECTURE/API docs.
...
A strong `EXTERNAL_DOCS.md` normally contains:
...
Owns `EXT-*`. TECHSTACK/ARCHITECTURE/SRS/API entries cite `EXT-*` instead of scattering raw URLs.

## Good Example Snippet

See `examples/external-docs-example.md`. External entries should be link-health aware:

```markdown
EXT-API-001 SQLite documentation
Last verified: 2026-05-24
Cross-refs: TS-DATA-01, C-01, FR-TASK-001.
```
