...
Use this guide when creating, extracting, updating, or reviewing `TESTCASES.md`. Also apply `reference/STYLE.md` for headers, IDs, versioning, traceability, and table conventions.

## Purpose and Audience

Defines executable verification coverage with steps, expected results, fixtures, priority, and automation type. Write for implementers, reviewers, and future agents who need clear scope, source evidence, and testable decisions.

## When to Include or Skip

Include `TESTCASES.md` when the selected archetype or change request needs this artifact. Skip it when it would only repeat another doc without adding implementation or verification value. If skipped, record the reason in PRD §Doc Set.

## Inputs to Read First

- SRS, use cases, user-story AC, business rules, existing test files, bug reports.
...
- Test cases must be executable: preconditions, data, steps, expected result, priority, type, and automation/manual status.
- Build coverage matrices for `FR-*`, `NFR-*`, `UC-*`, and `AC-*` when included.
- Link each workflow or sequence-diagram branch with at least one `TC-*` when that branch represents required behavior.
- Do not paraphrase requirements; specify observable verification.
- In brownfield mode, map existing test files/functions to `TC-*` IDs and identify uncovered behavior.
...
A strong `TESTCASES.md` normally contains:
...
Owns `TC-*`. Every `FR-*`/`NFR-*`/`UC-*` and included `AC-*` needs at least one `TC-*`.

## Good Example Snippet

See `examples/test-cases-example.md`. Executable tests name setup, action, and expected evidence:

```markdown
TC-TASK-001
Steps: submit valid task; open triage queue.
Expected: task appears Ready for assignment and audit event exists.
Workflow branch: UF-01 happy path.
```
