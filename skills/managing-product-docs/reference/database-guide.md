...
Use this guide when creating, extracting, updating, or reviewing `DATABASE.md`. Also apply `reference/STYLE.md` for headers, IDs, versioning, traceability, and table conventions.

## Purpose and Audience

Documents physical schema, ER relationships, indexes, roles, retention, migration notes, and backup/restore approach. Write for implementers, reviewers, and future agents who need clear scope, source evidence, and testable decisions.

## When to Include or Skip

Include `DATABASE.md` when the selected archetype or change request needs this artifact. Skip it when it would only repeat another doc without adding implementation or verification value. If skipped, record the reason in PRD §Doc Set.

## Inputs to Read First

- PRD entities, SRS behavior, architecture ownership, migrations, ORM models, schema dumps.
...
- Document physical reality: tables, columns, constraints, indexes, roles, migrations, retention, backups, and restore notes.
- In brownfield mode, copy schema semantics from migrations/models; do not redesign silently.
- Trace tables to PRD entities and SRS requirements. Trace audit/permissions to `BR-*` or `NFR-*`.
- Include Mermaid ER diagrams only when relationships are clearer visually than in tables.
...
A strong `DATABASE.md` normally contains:
...
Tables trace to PRD entities and SRS requirements; permission/audit behavior traces to `BR-*` or `NFR-*`.

## Good Example Snippet

See `examples/database-example.md`. Schema entries should trace constraints to requirements/rules:

```markdown
priority text check in Low, Normal, High, Release Blocker
Source: FR-TASK-001, BR-TASK-01.
```
