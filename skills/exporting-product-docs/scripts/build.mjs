  ...
  "node_modules/highlight.js/lib/core.js"(exports, module) {
    ...
    var HLJS = function(hljs) {
      ...
      function blockLanguage(block2) {
        ...
        return classes.split(/\s+/).find((_class4) => shouldNotHighlight(_class4) || getLanguage(_class4));
      }
...
function asciiTrim(str2) {
  let start = 0;
  for (; start < str2.length; start++) {
    if (!isAsciiTrimmable(str2.charCodeAt(start))) {
      break;
    }
  }
  let end = str2.length - 1;
  for (; end >= start; end--) {
    if (!isAsciiTrimmable(str2.charCodeAt(end))) {
      break;
    }
  }
...
function applyReplacements(str2, replacements) {
  let result = "";
  let lastPos = 0;
  replacements.sort((a, b) => a.pos - b.pos);
  for (let i = 0; i < replacements.length; i++) {
    const replacement = replacements[i];
    result += str2.slice(lastPos, replacement.pos) + replacement.ch;
    lastPos = replacement.pos + 1;
  }
  return result + str2.slice(lastPos);
}
...
for (const [name, lang] of Object.entries({
  ...
  core_default.registerLanguage(name, lang);
}
var scriptDir = path.dirname(fileURLToPath(import.meta.url));
var ASSETS_DIR = path.join(scriptDir, "..", "assets");
var MERMAID_PATH = path.join(scriptDir, "vendor", "mermaid.min.js");
var ORDER = [
  "prd",
  "techstack",
  "architecture",
...
var KNOWN_GROUP_KEYS = new Set(GROUPS.flatMap(([, keys]) => keys));
...
function normKey(file) {
  return path.basename(file).replace(/\.md$/i, "").replace(/[-_]?example$/i, "").toLowerCase().replace(/[^a-z0-9]/g, "");
}
...
var md = new lib_default({ html: true, linkify: false, typographer: false, highlight });
md.use(import_markdown_it_anchor.default, { slugify: (s) => slugger.slug(s), permalink: false, tabIndex: false });
function highlight(str2, lang) {
  if (lang === "mermaid") {
    return `<pre class="mermaid">${escapeHtml2(str2)}</pre>`;
  }
  if (lang && core_default.getLanguage(lang)) {
    try {
      return `<pre class="hljs"><code>${core_default.highlight(str2, { language: lang }).value}</code></pre>`;
    } catch {
    }
  }
  return `<pre class="hljs"><code>${escapeHtml2(str2)}</code></pre>`;
}
...
function extractDesignTokens(raw) {
  ...
  let data;
  try {
    data = jsYaml.load(lines.slice(start + 1, end).join("\n"));
  } catch {
...
function renderTokens(data) {
  const rows = flatten(data).map(([k, v]) => {
    ...
    return `<tr><td><code>${escapeHtml2(k)}</code></td><td>${swatch}${escapeHtml2(v)}</td></tr>`;
  }).join("\n");
...
function statusBadge(status) {
  ...
  return `<span class="badge badge-${cls}">${escapeHtml2(status)}</span>`;
}
function deriveBrand(docs) {
  const suffixes = docs.map((d) => d.title.includes(" \u2014 ") ? d.title.split(" \u2014 ").pop().trim() : null).filter(Boolean);
...
function navLinks(docs, hrefFor, activeHtmlName) {
  let out = "";
  const render = (label, list2) => {
    if (!list2.length) return "";
    const items = list2.map((d) => {
      const active = d.htmlName === activeHtmlName ? ' class="active"' : "";
      return `<li><a href="${hrefFor(d)}"${active}>${escapeHtml2(shortLabel(d))}</a></li>`;
    }).join("");
...
function headingsOf(bodyHtml) {
  return [...bodyHtml.matchAll(/<h([1-6])[^>]*\bid="([^"]+)"[^>]*>([\s\S]*?)<\/h\1>/g)].map((m) => ({ level: +m[1], anchor: m[2], text: stripTags(m[3]) }));
}
function pageShell({ title, brand, brandHref = "index.html", showSearch, sidebar, main, tocRail = "", headExtra, footScript }) {
  const search = showSearch ? `<div class="search"><input id="docsearch" type="search" placeholder="Search documentation\u2026" autocomplete="off" spellcheck="false" aria-label="Search"><div class="search-results" id="searchresults" hidden></div></div>` : '<div class="search-spacer"></div>';
  return `<!DOCTYPE html>
...
<header class="topbar">
  ...
  <a class="brand" href="${brandHref}">${escapeHtml2(brand)}</a>
  ${search}
...
function build() {
  ...
  const files = fs.readdirSync(inDir).filter((f) => f.toLowerCase().endsWith(".md")).map((f) => ({ file: f, key: normKey(f), abs: path.join(inDir, f) })).sort((a, b) => orderIndex(a.key) - orderIndex(b.key) || a.file.localeCompare(b.file));
  if (files.length === 0) {
...
<script src="assets/app.js"></script>`;
    ...
    docs.forEach((d, i) => {
      ...
      const prev = docs[i - 1];
      const next = docs[i + 1];
      const breadcrumb = `<nav class="breadcrumb"><a href="index.html">Docs</a><span>/</span><span>${escapeHtml2(groupOf(d.key))}</span><span>/</span><span class="current">${escapeHtml2(shortLabel(d))}</span></nav>`;
      const pager = `<nav class="pager">` + (prev ? `<a class="pager-prev" href="${prev.htmlName}"><span>Previous</span><strong>${escapeHtml2(shortLabel(prev))}</strong></a>` : `<span></span>`) + (next ? `<a class="pager-next" href="${next.htmlName}"><span>Next</span><strong>${escapeHtml2(shortLabel(next))}</strong></a>` : `<span></span>`) + `</nav>`;
...
${pager}`;
      fs.writeFileSync(
        ...
        pageShell({ title: `${shortLabel(d)} \xB7 ${brand}`, brand, showSearch: true, sidebar: sidebar(d.htmlName), main, tocRail, headExtra, footScript })
      );
    });
    const cards = docs.map(
      (d) => `<a class="doc-card" href="${d.htmlName}"><span class="doc-card-group">${escapeHtml2(groupOf(d.key))}</span><span class="doc-card-title">${escapeHtml2(shortLabel(d))}</span><span class="doc-card-meta"><span class="ver">v${escapeHtml2(d.version || "\u2014")}</span> ${statusBadge(d.status)}</span></a>`
    ).join("\n");
    const homeMain = `<article class="doc home">
<h1>${escapeHtml2(brand)} Documentation</h1>
